...
command_t commands[] =
    {{"match",
     ...
     {"multi-match",
      ...
      "allocate_orelse_reserve | without_allocating | without_allocating_future): "
                                 ...
                                 double elapse)
{
    ...
    if (matched) {
        ...
        // Only add the job to ctx->jobs if it was actually allocated
        if (allocated) {
                   ...
                   std::ostream &out)
{
    ...
    if (is_path == "true") {
        ...
        // Neither of the above remove calls will update counts, so we need to do it here.
        if (ctx->traverser->initialize () != 0) {
            std::cerr << "ERROR: failed to update counts after partial cancel and remove "
                      << std::endl;
            std::cerr << "ERROR: " << ctx->traverser->err_message ();
            return -1;
        }
