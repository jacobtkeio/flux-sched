...
TEST_CASE ("Match basic jobspec", "[match C]")
{
    ...
    SECTION ("MATCH_WITHOUT_ALLOCATING_EXTEND")
    {
        ...
        REQUIRE (reserved == false);
        REQUIRE (at == 0);
        REQUIRE ((match_json = json_loads (R, JSON_DECODE_ANY, NULL)) != nullptr);
    ...
    SECTION ("Match multiple resources per request")
    {
        ...
        REQUIRE (Rstr.find ("\"starttime\": 3600"));
        linestart = 1 + Rstr.find ("\n", linestart);
        REQUIRE (linestart != Rstr.npos);

        REQUIRE (Rstr.find ("\"starttime\": 7200", linestart));
        linestart = 1 + Rstr.find ("\n", linestart);
        REQUIRE (linestart != Rstr.npos);

        REQUIRE (Rstr.find ("\"starttime\": 10800", linestart));
        linestart = 1 + Rstr.find ("\n", linestart);
        REQUIRE (linestart != Rstr.npos);

        REQUIRE (Rstr.find ("\"starttime\": 14400", linestart));
        linestart = 1 + Rstr.find ("\n", linestart);
        REQUIRE (linestart != Rstr.npos);
        ...
        REQUIRE (Rstr.find ("\"expiration\": 9223372036854775807") != Rstr.npos);
        REQUIRE (Rstr.find ("\n", 1 + Rstr.find ("\n")) == Rstr.npos);
    }
}
