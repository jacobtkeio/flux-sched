...
TEST_CASE ("Match basic jobspec", "[match C++]")
{
    ...
    SECTION ("Match multiple resources per request")
    {
        ...
        CHECK (R.find ("\"starttime\": 3600"));
        linestart = 1 + R.find ("\n", linestart);
        REQUIRE (linestart != R.npos);

        CHECK (R.find ("\"starttime\": 7200", linestart));
        linestart = 1 + R.find ("\n", linestart);
        REQUIRE (linestart != R.npos);

        CHECK (R.find ("\"starttime\": 10800", linestart));
        linestart = 1 + R.find ("\n", linestart);
        REQUIRE (linestart != R.npos);

        CHECK (R.find ("\"starttime\": 14400", linestart));
        linestart = 1 + R.find ("\n", linestart);
        REQUIRE (linestart != R.npos);
        ...
        CHECK (R.find ("\"expiration\": 9223372036854775807") != R.npos);
        REQUIRE (R.find ("\n", 1 + R.find ("\n")) == R.npos);
    }
}
