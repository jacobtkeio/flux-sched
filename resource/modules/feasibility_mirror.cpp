...
// N.B. the topic of the first element is set in init_module
static struct flux_msg_handler_spec htab[] =
    {{FLUX_MSGTYPE_REQUEST, nullptr, feasibility_request_cb, 0}, FLUX_MSGHANDLER_TABLE_END};
...
static std::shared_ptr<resource_ctx_t> getctx (flux_t *h)
{
    ...
    const char *name = static_cast<const char *> (flux_aux_get (h, "flux::name"));
    if ((d = flux_aux_get (h, name)) != NULL)
...
static std::shared_ptr<resource_ctx_t> init_module (flux_t *h, int argc, char **argv)
{
    ...
    const std::string feasibility_topic = std::string (ctx->m_module_name).append (".check");
    htab[0].topic_glob = feasibility_topic.c_str ();
...
static int init_resource_graph (std::shared_ptr<resource_ctx_t> &ctx)
{
    ...
    const std::optional<std::string> &mirror_uri = opts.get_mirror_uri ();
    ...
    rc += required_module_option (ctx, mirror_uri, "mirror-uri");
    if (rc != 0)
        return rc;
...
extern "C" int mod_main (flux_t *h, int argc, char **argv)
{
    ...
    try {
        ...
        flux_aux_set (h, ctx->m_module_name, &ctx, NULL);
        flux_log (h, LOG_DEBUG, "%s: %s module starting", __FUNCTION__, ctx->m_module_name);
