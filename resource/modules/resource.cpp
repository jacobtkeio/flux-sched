...
static void match_request_cb (flux_t *h, flux_msg_handler_t *w, const flux_msg_t *msg, void *arg)
{
    ...
    if (num_matches <= 0) {
        errno = EINVAL;
        flux_log_error (h, "%s: invalid requested number of matches", __FUNCTION__);
        goto error;
    }
    if (run_match (ctx, jobid, cmd, js_str, num_matches, &now, &at, &overhead, R, NULL) < 0) {
    ...
    if (std::string ("without_allocating") == cmd || std::string ("without_allocating_extend") == cmd) {
        status = "MATCHED";
    } else {
