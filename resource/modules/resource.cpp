...
static void add_subgraph_request_cb (flux_t *h,
                                     flux_msg_handler_t *w,
                                     const flux_msg_t *msg,
                                     void *arg);
                                     ...
                                     void *arg)
{
    ...
    if (flux_request_unpack (msg, NULL, "{s:s}", "subgraph", &R_subgraph) < 0)
        goto error;
                                   ...
                                   void *arg)
{
    ...
    char *rank_list_str = nullptr;
    char *msg_rank_list_str;
    bool is_up_req;
    bool is_down_req;
    ...
    ctx->m_resources_down_updated = true;
    // respond to the set-status request with success
    if (flux_respond (h, msg, NULL) < 0) {
        flux_log_error (h, "%s: flux_respond", __FUNCTION__);
    }
